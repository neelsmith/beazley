...
class FindsSourceSpec extends FlatSpec {
  ...
  "The FindsSource object"  should "be able to add geography for a single Find from a short list in delimited-text form" in {
    ...
    val fnd = FindsSource.findFromShortList(oneEntry, ",")
    fnd match {
      case f: Find => {
